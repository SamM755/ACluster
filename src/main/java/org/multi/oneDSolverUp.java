...
public class oneDSolverUp {
    ...
    public static Object[] optimalKMedoid1D(long[][] data, int k, int dim) {
        ...
        int[] finalSizes = new int[finalMedoids.length];
        for(int i = 0; i < finalSizes.length; i++){
            finalSizes[i] = clusterSizes.get(i);
        }
    ...
    private static long[][] precomputeCosts(IndexedValue[] sortedData) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            prefixSum[i + 1] = prefixSum[i] + sortedData[i].value;
        }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i; j < n; j++) {
                ...
                long leftSum = prefixSum[medianIndex + 1] - prefixSum[i];
                long leftCount = medianIndex - i + 1;
                long leftCost = (leftCount * medianValue) - leftSum;

                
                
                
                long rightSum = prefixSum[j + 1] - prefixSum[medianIndex + 1];
                long rightCount = j - medianIndex;
                long rightCost = rightSum - (rightCount * medianValue);
    ...
    private static Object[] sortResults(long[][] medoids, int[] clusterAssignment, int[] clusterSizes) {
        ...
        if (clusterSizes == null || clusterSizes.length == 0) {
            ...
            int[] tempSizes = new int[k];
            
            for(int assignment : clusterAssignment){
                if(assignment >= 0 && assignment < k)
                    tempSizes[assignment]++;
            }
            clusterSizes = tempSizes;
        }
        ...
        int[] sortedClusterSizes = new int[k];
        for (int i = 0; i < k; i++) {
        ...
        for (int i = 0; i < clusterAssignment.length; i++) {
            if (clusterAssignment[i] >= 0 && clusterAssignment[i] < k) {
