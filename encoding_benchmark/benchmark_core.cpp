...
void run_cluster_encoder_test(
    ...
    auto apply_group_size = [&](auto& comp) {
        ...
        if constexpr (std::is_same_v<T, KClusterSub>) {
    ...
    for(int run = 0; run < NUM_RUNS; ++run) {
        ...
        if (run == 0) {
            using C = std::decay_t<ClusterCompressor>;
            if constexpr (std::is_same_v<C, KClusterSub>) {
                int sub_mode = -1;
                int sub_strategy = -1;
        ...
        if (run == 0) {
            ...
            using C = std::decay_t<ClusterCompressor>;
            if constexpr (std::is_same_v<C, KClusterSub>) {
                verify_rows_ordered(original_rows, decoded_rows, dataset_name);
            } else {
                verify_data_order_oblivious(original_rows, decoded_rows, dataset_name);
            }
            std::cout << "Verification successful!\n";
...
int legacy_experiment_catalog() {
    try {
        ...
        // Runs the subspace grouping experiment for KClusterSub.
...
template void run_columnar_encoder_test<Chimp>(const std::string&, int, int, const std::string&, const std::string&, SecondaryCompressor*);
template void run_columnar_encoder_test<Elf>(const std::string&, int, int, const std::string&, const std::string&, SecondaryCompressor*);
template void run_columnar_encoder_test<Gorilla>(const std::string&, int, int, const std::string&, const std::string&, SecondaryCompressor*);
template void run_columnar_encoder_test<Rle>(const std::string&, int, int, const std::string&, const std::string&, SecondaryCompressor*);
template void run_columnar_encoder_test<Huffman>(const std::string&, int, int, const std::string&, const std::string&, SecondaryCompressor*);
template void run_columnar_encoder_test<Alp>(const std::string&, int, int, const std::string&, const std::string&, SecondaryCompressor*);
template void run_cluster_encoder_test<KCluster>(const std::string&, int, int, const std::string&, const std::string&, SecondaryCompressor*, int);
template void run_cluster_encoder_test<ACluster>(const std::string&, int, int, const std::string&, const std::string&, SecondaryCompressor*, int);
template void run_cluster_encoder_test<KClusterKMeans>(const std::string&, int, int, const std::string&, const std::string&, SecondaryCompressor*, int);