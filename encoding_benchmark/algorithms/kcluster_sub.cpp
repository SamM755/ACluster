...
uint32_t read_u32(const std::vector<uint8_t>& in, size_t& pos) {
    if (pos + 4 > in.size()) throw std::runtime_error("KClusterSub invalid stream");
...
uint64_t read_uvarint(const std::vector<uint8_t>& in, size_t& pos) {
    ...
    int shift = 0;
    while (true) {
        if (pos >= in.size() || shift > 63) throw std::runtime_error("KClusterSub invalid varint");
        uint8_t b = in[pos++];
...
std::vector<uint32_t> unpack_bits(const std::vector<uint8_t>& bytes, size_t expected, int bw) {
    ...
    while (out.size() < expected) {
        while (acc_bits < bw) {
            if (p >= bytes.size()) throw std::runtime_error("KClusterSub unpack underflow");
...
std::vector<uint32_t> decode_ids(const std::vector<uint8_t>& in) {
    ...
    while (ids.size() < total) {
        ...
        if (pos + packed_sz > in.size()) throw std::runtime_error("KClusterSub decode ids truncated");
        ...
        if (cnt > block_size) throw std::runtime_error("KClusterSub decode ids bad block");
    }
    return ids;
}
...
std::vector<uint32_t> decode_positions(const std::vector<uint8_t>& in, uint32_t row_count) {
    if (in.empty()) throw std::runtime_error("KClusterSub decode positions empty");
    uint8_t mode = in[0];
    std::vector<uint8_t> body(in.begin() + 1, in.end());
    if (mode == 0) {
        auto pos = decode_ids(body);
        for (uint32_t p : pos) if (p >= row_count) throw std::runtime_error("KClusterSub position out of range");
        return pos;
    }
    if (mode == 1) {
        ...
        for (size_t i = 0; i < deltas.size(); ++i) {
            ...
            if (p < 0 || p >= static_cast<int64_t>(row_count)) throw std::runtime_error("KClusterSub delta position out of range");
            pos.push_back(static_cast<uint32_t>(p));
        }
        return pos;
    }
    throw std::runtime_error("KClusterSub decode positions mode unknown");
}
...
std::vector<uint8_t> encode_ambiguous_positions(
    ...
    for (size_t i = 0; i < dec1.size(); ++i) {
        ...
        if (it == b.end()) throw std::runtime_error("KClusterSub ambiguous encode mapping miss");
...
std::vector<uint32_t> decode_ambiguous_positions(
    ...
    if (p + idx_sz > ambiguous_stream.size()) throw std::runtime_error("KClusterSub ambiguous stream truncated");
    ...
    bucket.reserve(dec0.size() * 2);
    for (uint32_t i = 0; i < static_cast<uint32_t>(dec0.size()); ++i) bucket[quant_key(dec0[i][anchor_idx_in_g0])].push_back(i);
    std::unordered_map<long long, size_t> amb_counter;
    ...
    for (size_t i = 0; i < dec1.size(); ++i) {
        long long k = quant_key(dec1[i][anchor_idx_in_g1plus]);
        const auto& b = bucket[k];
        if (b.empty()) throw std::runtime_error("KClusterSub ambiguous decode bucket empty");
        if (b.size() == 1) {
            pos1.push_back(b[0]);
            continue;
        }
        if (idx_ptr >= idx.size()) throw std::runtime_error("KClusterSub ambiguous decode idx underflow");
        uint32_t li = idx[idx_ptr++];
        if (li >= b.size()) throw std::runtime_error("KClusterSub ambiguous decode local idx out of range");
        pos1.push_back(b[li]);
    }
    if (idx_ptr != idx.size()) throw std::runtime_error("KClusterSub ambiguous decode idx overflow");
    for (uint32_t p1 : pos1) if (p1 >= row_count) throw std::runtime_error("KClusterSub ambiguous decode pos out of range");
    return pos1;
}
...
std::string make_group_csv(
    ...
    std::filesystem::path tmp = std::filesystem::temp_directory_path() / ("kclustersub_g_" + tag + ".csv");
    std::ofstream out(tmp.string());
    if (!out.is_open()) throw std::runtime_error("KClusterSub cannot create temp file");
...
std::vector<uint32_t> map_rows(
    ...
    for (const auto& r : decoded) {
        ...
        if (it == buckets.end() || it->second.empty()) throw std::runtime_error("KClusterSub mapping failed");
...
std::vector<uint8_t> KClusterSub::encode_multidim(
    ...
    if (groups.empty()) throw std::runtime_error("KClusterSub empty groups");
    ...
    std::vector<uint8_t> out;
    out.push_back('K'); out.push_back('S'); out.push_back('U'); out.push_back('B');
    out.push_back(1);
    out.push_back(1);
...
std::vector<std::vector<double>> KClusterSub::decode_multidim(const std::vector<uint8_t>& compressed_data) {
    if (compressed_data.size() < 7) throw std::runtime_error("KClusterSub decode stream too small");
    if (!(compressed_data[0] == 'K' && compressed_data[1] == 'S' && compressed_data[2] == 'U' && compressed_data[3] == 'B')) {
        throw std::runtime_error("KClusterSub decode bad magic");
    }
    uint8_t mode = compressed_data[4];
    size_t pos = 5;
    if (mode != 1) throw std::runtime_error("KClusterSub decode unsupported mode");
    ...
    if (strategy == 1) {
        ...
        if (pos + centroid_sz + assign_sz > compressed_data.size()) throw std::runtime_error("KClusterSub decode strategy1 truncated");
        ...
        for (int i = 0; i < group_count; ++i) {
            if (pos + residual_sz[i] > compressed_data.size()) throw std::runtime_error("KClusterSub decode residual truncated");
        ...
        for (int gi = 0; gi < group_count; ++gi) {
            ...
            for (int r = 0; r < row_count; ++r) {
                ...
                if (cid < 0 || cid >= k_global) throw std::runtime_error("KClusterSub decode bad centroid id");
    ...
    if (strategy != 0) throw std::runtime_error("KClusterSub decode unsupported strategy");
    ...
    read_u64(compressed_data, pos);
    read_u64(compressed_data, pos);
    if (group_count <= 0) throw std::runtime_error("KClusterSub decode bad group count");
    ...
    for (int i = 0; i < group_count; ++i) {
        if (pos + blob_sizes[i] > compressed_data.size()) throw std::runtime_error("KClusterSub decode blob truncated");
    ...
    for (int i = 0; i < group_count - 1; ++i) {
        if (pos + pos_sizes[i] > compressed_data.size()) throw std::runtime_error("KClusterSub decode pos truncated");
    ...
    if (dec_groups[0].size() != static_cast<size_t>(row_count)) throw std::runtime_error("KClusterSub decode row mismatch");
    ...
    for (int gi = 1; gi < group_count; ++gi) {
        ...
        if (dec_groups[gi].size() != pos_map.size()) throw std::runtime_error("KClusterSub decode group size mismatch");
        std::vector<uint8_t> seen(row_count, 0);
        for (size_t r = 0; r < dec_groups[gi].size(); ++r) {
            uint32_t p = pos_map[r];
            if (p >= static_cast<uint32_t>(row_count) || seen[p]) throw std::runtime_error("KClusterSub decode mapped row invalid");
            seen[p] = 1;
